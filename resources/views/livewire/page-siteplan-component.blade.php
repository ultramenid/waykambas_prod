<div x-data="{lang:'english'}" class="max-w-6xl mx-auto px-6 md:px-8 min-h-screen">
    ...
    <div class=" border-b border-gray-300 dark:border-opacity-20 py-16 mb-6">
        <div class="max-w-6xl mx-auto px-6  flex justify-between ">
            <h1 class="sm:text-4xl text-xl text-newgray-900 dark:text-newgray-300 font-semibold ">Site Plan</h1>
            <div class="z-30">
                <button wire:loading.remove wire:target='storesiteplan'  wire:click='storesiteplan' id="btnStore" class="inline-flex sm:px-16 px-8 sm:py-2 py-1 rounded dark:hover:bg-newgray-900 dark:hover:border-gray-200 dark:hover:text-gray-200 hover:bg-white hover:text-newgray-900 border hover:border-newgray-900 bg-newgray-900 dark:bg-gray-100 text-newgray-100 dark:text-newgray-900">
                    Save
                </button>
                <button wire:loading wire:target='storesiteplan' id="btnStore" class="inline-flex sm:px-16 px-8 sm:py-2 py-1 rounded dark:hover:bg-newgray-900 dark:hover:border-gray-200 dark:hover:text-gray-200 hover:bg-white hover:text-newgray-900 border hover:border-newgray-900 bg-newgray-900 dark:bg-gray-100 text-newgray-100 dark:text-newgray-900">
                    ...
                    plugins: [
                            ...
                            setup: function(editor) {
                                editor.on('change', function(e) {
                                    @this.set('textsiteplanEN', editor.getContent());
                            });
                        }
                });">
                <textarea rows="20" id="textsiteplanEN" name="textsiteplanEN"  wire:model.defer='textsiteplanEN' required></textarea>
            </div>
        </div>
    </div>
            ...
            <div class="w-full border border-gray-300 dark:border-opacity-20 rounded"
                ...
                convert_urls : true,
                    plugins: [
                            ...
                            setup: function(editor) {
                                editor.on('change', function(e) {
                                    @this.set('textsiteplanID', editor.getContent());
                            });
                        }
                });">
                <textarea rows="20" id="textsiteplanID" name="textsiteplanID"  wire:model.defer='textsiteplanID' required></textarea>
            </div>
        </div>
    </div>
</div>
